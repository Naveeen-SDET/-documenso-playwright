...
test.describe('@smoke @auth login flows', () => {
  // These tests need a fresh unauthenticated session
  test.use({ storageState: { cookies: [], origins: [] } });

  test('successful login redirects away from signin', async ({ page }, testInfo) => {
    // Skip in CI environment OR when running the ci Playwright project (no pre-seeded account)
   test.skip(true, 'Login success covered by auth setup — skipped to avoid rate limit interference');
